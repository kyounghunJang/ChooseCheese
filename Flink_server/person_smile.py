...
class Choosecheese:
    ...
    def search_and_add_users_by_image(self,user_id,image_data):
        ...
        try:
            #원본 이미지 저장 
            last_file_name=self.get_last_file_in_folder(user_id,'capstone-bucket-original')+1
            self.s3.put_object(
        ...
        ## YOLO FRAME DETECT -> YOLO SERVER 필요해 13ms CROP -> IMAGE 
            ...
            # 각 얼굴에 대해 사용자 검색
            #last_member=self.get_last_file_in_folder(user_id,'capstone-bucket-pdb')
            current_time = datetime.now().strftime("%Y%m%d%H%M%S")
            # 타임스탬프와 UUID를 결합하여 고유한 파일 이름 생성
            member = uuid.uuid5(uuid.NAMESPACE_DNS, current_time)
            for face in response['FaceDetails']:
                ...
                if match_faceid==False:
                    last_member+=1
                    self.add_face_to_collection(user_id, crop_face, member)
                    self.s3.put_object(
                        Bucket='capstone-bucket-pdb',
                        Key=user_id+'/'+str(member)+'.jpg',
                        Body=crop_face
                    )
                    tmp_mem.append(member)
                    member = uuid.uuid5(uuid.NAMESPACE_DNS, current_time)
                else:
            ...
            dic['miso_point']={'N':int(smile_value)}
            dic['detected_count']={'N':len(tmp_mem)}
            dic['originalS3']={'S':'https://capstone-bucket-original.s3.ap-northeast-2.amazonaws.com/'+str(last_file_name)+'.jpg'}
